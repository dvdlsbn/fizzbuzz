...
class FirstFragment : Fragment() {
            ...
            if (isFormValid) {
                findNavController()
                    .navigate(R.id.action_FirstFragment_to_SecondFragment,
                        Bundle().apply {
                            firstInt?.let { putInt(FIRST_INT.name, if (it == 0) 1 else it) }
                            secondInt?.let { putInt(SECOND_INT.name, if (it == 0) 1 else it) }
                            fizz?.let { putString(FIZZ_STRING.name, it) }
                            buzz?.let { putString(BUZZ_STRING.name, it) }
                            limit?.let {
                                putInt(
                                    LIMIT_INT.name,
                                    if (it > 10000) 10000 else if (it <= 1) 1 else it
                                )
                            }
                        }
                    )
    ...
    private fun checkFormValidity() {
        if (firstInt == null || secondInt == 0) {
            isFormValid = false
            binding.firstInt.error = getString(R.string.mandatory_and_not_null_field)
        }
        if (secondInt == null || secondInt == 0) {
            isFormValid = false
            binding.secondInt.error = getString(R.string.mandatory_and_not_null_field)
